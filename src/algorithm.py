...
def printPath(path):
    ...
    while not path.isEmpty():
        print(path.pop().get_pos())
        length += 1
    print("Goal\nLength:", length, "steps")


def algorithm(key, draw, grid, start, end):
    if key == "astar":
    ...
    while not open_set.empty():
        ...
        for neighbor in current.neighbors:
            ...
            if alt < dist[neighbor]:
                ...
                if neighbor not in open_set_hash:
                    count += 1
                    open_set.put((alt, count, neighbor))
                    open_set_hash.add(neighbor)
                    neighbor.make_open()
...
def reset(WINDOW, grid, rows, width):
    WINDOW.fill(WHITE)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and start and end:
                    ...
                    algorithm(key, lambda: draw(WINDOW, grid, ROWS, width), grid, start, end)
